window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const en = {
			...
			"card.history.desc": "Archive browsing and cleanup (Records Room). When off, history routes and the tab are gone.",
			...
			"panel.tab.history": "Records Room",
		...
		*/
		...
		function HealthPanel({ t, onClose }) {
			...
			const runScan = () => {
				...
				setDetail(null);
				setDischarge(null);
				callHostAny("session-health-scan", {
					limit: 30,
					onlyProblems: true
				}, 12e4).then((res) => {
					if (res.ok === true) setFindings(res.findings ?? []);
					else setError(res.error ?? "体检失败");
				}).catch((err) => setError(String(err instanceof Error ? err.message : err))).finally(() => setScanning(false));
			};
			...
			if (report !== void 0) lines.push((0, react.createElement)("div", {
				...
				if (findings.length === 0) lines.push((0, react.createElement)("div", {
					...
					className: "dss_list",
					role: "list"
				}, findings.map((item) => (0, react.createElement)("li", {
					key: item.sessionId,
					className: "dss_row"
				}, [
					(0, react.createElement)("span", {
